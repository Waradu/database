<template>
  <div class="table_id page">
    <header class="default">
      ...
      <div class="tags">
        <div
          v-for="tag in databaseStore.getTableTags(table.id.toString())"
          :key="tag.id"
          class="tag"
          :style="{
            '--color': tag.color + '50',
...
const filteredRows = computed(() => {
  ...
  return databaseStore
    .getTableRows(tableId.value)
    .filter(
      (row) =>
        row.name.toLowerCase().includes(searchText) ||
        databaseStore
          .getRowTags(row.id.toString())
          .some((tag) => tag.name.toLowerCase().includes(searchText))
    );
});
